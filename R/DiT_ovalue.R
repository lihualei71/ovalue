...
dempster <- function(n, a, b){
    ...
    tmp2 <- pmin(pmax(tmp2, 1e-10), 1-1e-10)
    fac2 <- -lgamma(n - tmp1 + 1) - lgamma(tmp1 + 1) +
        (tmp1 - 1) * log(tmp2) + (n - tmp1) * log(1 - tmp2)
    sum(exp(fac1 + fac2))
}
...
dempster_lower_beta1 <- function(n, alpha, beta0){
    ...
    lower <- max(log(1 / alpha) / 2 / n / beta0 - beta0 - 1e-4, 1e-2)
    upper <- 1
    while (TRUE){
        if (is.nan(find_beta1(lower))){
            browser()
        }
        if (find_beta1(upper) > 0){
            lower <- upper
            upper <- 2 * upper
        } else if (find_beta1(lower) < 0){
            upper <- lower
            lower <- lower / 2
        } else {
            break
